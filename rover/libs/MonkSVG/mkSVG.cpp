...
namespace MonkSVG {
	...
	bool SVG::handle_xml_element( TiXmlElement* element ) {
        ...
        {
            handle_circle(element);
        }else if (type == "ellipse")
        {
            handle_ellipse(element);
        }
        else if( type == "symbol" ) {
			string id;
...
//		}
    ...
    void SVG::handle_circle(TiXmlElement* pathElement)
    {
		_handler->onPathBegin();
        cout << "onPathBegin();" << endl;
        
		float pos[3];
		pathElement->QueryFloatAttribute( "cx", &pos[0] );
		pathElement->QueryFloatAttribute( "cy", &pos[1] );
		pathElement->QueryFloatAttribute( "r", &pos[2] );
        
        _handler->onPathEllipse(pos[0], pos[1], pos[2]*2, pos[2]*2);
        cout << "onPathEllipse(" << pos[0] << "," << pos[1] << "," << pos[2]*2 << "," << pos[2]*2 << ");" << endl;
    ...
    void SVG::handle_ellipse(TiXmlElement* pathElement)
    {
		_handler->onPathBegin();
        cout << "onPathBegin();" << endl;
		...
		pathElement->QueryFloatAttribute( "cx", &pos[0] );
		pathElement->QueryFloatAttribute( "cy", &pos[1] );
		pathElement->QueryFloatAttribute( "rx", &pos[2] );
		pathElement->QueryFloatAttribute( "ry", &pos[3] );
        
        _handler->onPathEllipse(pos[0], pos[1], pos[2]*2, pos[3]*2);
        cout << "onPathEllipse(" << pos[0] << "," << pos[1] << "," << pos[2]*2 << "," << pos[3]*2 << ");" << endl;
	...
	void SVG::handle_rect( TiXmlElement* pathElement )
    {
        ...
        cout << "onPathRect(" << pos[0] << "," << pos[1] << "," << sz[0] << "," << sz[1] << ");" << endl;
	...
	void SVG::handle_general_parameter( TiXmlElement* pathElement ) {
		...
		if ( pathElement->QueryStringAttribute( "fill", &fill ) == TIXML_SUCCESS ) {
            ...
            cout << "onPathFillColor(" << string_hex_color_to_uint( fill ) << "u);" << endl;
		}
		...
		if ( pathElement->QueryStringAttribute( "stroke", &stroke) == TIXML_SUCCESS ) {
            ...
            cout << "onPathStrokeColor(" << string_hex_color_to_uint( stroke ) << "u);" << endl;
		}
		...
		if ( pathElement->QueryStringAttribute( "stroke-width", &stroke_width) == TIXML_SUCCESS ) {
            ...
            cout << "onPathStrokeWidth(" << width << ");" << endl;
		}
		...
		if ( pathElement->QueryStringAttribute( "fill-rule", &fillrule) == TIXML_SUCCESS ) {
            ...
            cout << "onPathFillRule(\"" << fillrule << "\");" << endl;
		}
...
//			}
			...
			if ( islower(*state) ) {	// if lower case then relative coords (see SVG spec)
                ...
                cout << "setRelative( true );" << endl;
			} else {
				_handler->setRelative( false );
                cout << "setRelative( false );" << endl;
			}
		}
	...
	void SVG::parse_path_transform( string& tr )	{
		...
		if ( p != string::npos ) {
            ...
            cout << "onTransformTranslate(" << x << "," << y << ");" << endl;
            ...
            cout << "onTransformMatrix(" << a << "," << b << "," << c << "," << d << "," << e << "," << f << ");" << endl;
		}
	}
		...
		while ( /**c &&*/ state != 'e' ) {
			...
			switch ( state ) {
				...
				case 'H':
				{
                    ...
                    cout << "onPathHorizontalLine(" << x << ");" << endl;
					nextState(&c, &state);
				...
				case 'V':
				{
                    ...
                    cout << "onPathVerticalLine(" << y << ");" << endl;
					nextState(&c, &state);
				...
				case 'C':
				{
                    ...
                    cout << "onPathCubic(" << x1 << "," << y1 << "," << x2 << "," << y2 << "," << x3 << "," << y3 << ");" << endl;
					nextState(&c, &state);
                ...
                case 'S':
				{
                    ...
                    cout << "onPathSCubic(" << x2 << "," << y2 << "," << x3 << "," << y3 << ");" << endl;
					nextState(&c, &state);
				...
				case 'A':
				{
                    ...
                    cout << "onPathArc(" << rx << "," << ry << "," << x_axis_rotation << "," << large_arc_flag << "," << sweep_flag << "," << x << "," << y << ");" << endl;
					nextState(&c, &state);
                ...
                case 'Q':
                {
                    ...
                    cout << "onPathQuad(" << x1 << "," << y1 << "," << x2 << "," << y2 << ");" << endl;
                    nextState(&c, &state);
                }
                break;
	...
	void SVG::parse_path_style( string& ps ) {
		...
		if ( kv != style_key_values.end() ) {
			if( kv->second != "none" )
                ...
                cout << "onPathFillColor(" << string_hex_color_to_uint( kv->second ) << "u);" << endl;
		}
		...
		if ( kv != style_key_values.end() ) {
			if( kv->second != "none" )
                ...
                cout << "onPathStrokeColor(" << string_hex_color_to_uint( kv->second ) << "u);" << endl;
		}
		...
		if ( kv != style_key_values.end() ) {
            ...
            cout << "onPathFillRule(" << kv->second << ");" << endl;
		}
		...
		if ( kv != style_key_values.end() ) {
            ...
            cout << "onPathStrokeOpacity(" << o << ");" << endl;
		}
